        ...
        if (!empty($comment)) {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    $comments_query = $con->prepare("SELECT c.comment_text, u.fname, u.lname FROM comments c JOIN user_registration u ON c.user_no = u.user_no WHERE c.post_id = ? ORDER BY c.timestamp DESC");
                    ...
                    while ($row = $comments->fetch_assoc()) {
                        $comments_html .= '<div class="comment"><strong>' . htmlspecialchars($row['fname'] . ' ' . $row['lname']) . ':</strong> ' . htmlspecialchars($row['comment_text']) . '</div>';
                    }
