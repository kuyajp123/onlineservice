...
$query = "SELECT COUNT(*) AS reacted FROM heart_reactions WHERE post_id = ? AND user_no = ?";
$stmt = $con->prepare($query);
$stmt->bind_param('ii', $post_id, $_SESSION['user_no']);
$stmt->execute();
$result = $stmt->get_result();
$data = $result->fetch_assoc();
$userReacted = $data['reacted'] > 0;

// Get the total count of reactions for the post
$query = "SELECT COUNT(*) AS total_reactions FROM heart_reactions WHERE post_id = ?";
$stmt = $con->prepare($query);
$stmt->bind_param('i', $post_id);
$stmt->execute();
$result = $stmt->get_result();
$data = $result->fetch_assoc();
$reactionCount = $data['total_reactions'];
?>
        ...
        <div class="container-fluid thethree">
            <div class="container-fluid puso">
                <button type="button" class="heart-btn" data-post-id="<?php echo htmlspecialchars($post_id); ?>" data-user-no="<?php echo htmlspecialchars($loggedInUserNo); ?>">
                    <i class="fa-regular fa-heart <?php echo $userReacted ? 'fa-solid' : ''; ?>" style="color: <?php echo $userReacted ? '#ff0000' : '#000000'; ?>;"></i>
                </button>
                <span id="reaction-count"><?php echo htmlspecialchars($reactionCount); ?></span>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  heartButton.addEventListener('click', function () {
    const isReacted = heartIcon.classList.contains('fa-solid');

    fetch('scripts/toggle_reaction.php', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      body: new URLSearchParams({
        post_id: postId,
        action: isReacted ? 'unreact' : 'react'
      })
    })
    .then(response => response.json())
    .then(data => {
      if (data.status === 'success') {
        // Update the icon and count based on the reaction state
        if (isReacted) {
        ...
        reactionCountElement.textContent = data.newCount;
      } else {
        console.error('Error:', data.message);
      }
    })
